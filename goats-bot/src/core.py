...
from src.utils import (log, read_config, countdown_timer,
                        hju, mrh, htm, kng, bru, pth, _banner,
                          _clear)
...
class GoatsBot:
    ...
    async def refresh_token(self) -> bool:
        ...
        resp = await self.http.post(url, json={"refreshToken": token_data})
        resp = json.loads(await resp.text())
        if resp.get("statusCode"):
            log(f"Error refreshing token | {resp['message']}")
            return False

        access_token = resp["tokens"]["access"]["token"]
        refresh_token = resp["tokens"]["refresh"]["token"]
        self.access_token_expiry = int(datetime.fromisoformat(resp["tokens"]["access"]["expires"].replace("Z", "+00:00")).timestamp())
        self.http.headers["Authorization"] = f"Bearer {access_token}"
    ...
    async def user_data(self) -> dict:
        if not self.access_token:
            log.debug("No access token available. Logging in...")
        ...
        if await self.is_token_expired():
            ...
            if not await self.refresh_token():
                log.error("Failed to refresh token, re-logging in.")
                if not await self.login():
                    log.error("Failed to log in after token refresh.")
                    return {}
        headers = {
        ...
        try:
            async with self.http.get("https://api-me.goatsbot.xyz/users/me", headers=headers) as resp:
                ...
                if 'application/json' not in content_type:
                    resp_text = await resp.text()
                    log.error(f"Unexpected content type: {content_type}, Response text: {resp_text}")
                    return {}

                resp_json = await resp.json()
                if resp_json.get("statusCode"):
                    log.error(f"Error getting profile data | {resp_json['message']}")
                    return {}

                return resp_json

        except Exception as e:
            log.error(f"Exception while getting user data: {e}")
            return {}
    ...
    async def complete_mission(self, mission_data: dict) -> bool:
        ...
        if resp.get("statusCode"):
            ...
            if resp.get("status"):
                log(pth + f"{mission_data['name']} {hju}completed | Reward: {pth}{mission_data['reward']}")
                return True
            else:
    ...
    async def run(self):
        ...
        if user_data:
            ...
            log(hju + f"Balance: {pth}{user_data.get('balance')} ")
            log(hju + f"Telegram Age: {pth}{user_data.get('age')} {hju}years")
...
async def main():
    ...
    while True:
        ...
        for i, auth_data in enumerate(accounts, start=1):
            log(bru + f"Account {pth}{i}/{len(accounts)}")
            log(pth + f"~" * 38)
